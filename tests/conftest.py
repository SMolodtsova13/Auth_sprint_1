...
async def make_post_request(http_client):
    async def _inner(path: str, data: dict, headers: dict | None = None):
        ...
        if path == REGISTER_URL or path == LOGIN_URL or path == f"{LOGIN_URL}" or path == LOGIN_URL:
            if path == LOGIN_URL:
                resp = await http_client.post(base + path, data={'username': data['login'], 'password': data['password']}, headers=headers)
            else:
...
async def get_superuser_token(make_post_request, get_superuser_data):
    ...
    resp = await make_post_request(LOGIN_URL, { 'login': get_superuser_data['login'], 'password': get_superuser_data['password'] })
    data = await resp.json()
    return data['access_token']

@pytest_asyncio.fixture
async def make_post_request_with_roles(make_post_request, get_superuser_token):
